...
// ── 탭1: 다음열차 ─────────────────────────────────────────────
function NextTab({nowM, nowSec, hol, onStnChange}) {
  const [stn,setStn]=useState('동대구');
...
// ── 탭2: 시간표 ──────────────────────────────────────────────
function TTTab({nowM,hol}) {
  const [stn,setStn]=useState('동대구');
  const [dir,setDir]=useState('up');
  const ref=useRef(null);
  const rows=getTT(dir,hol)
    .map(t=>({...t,stM:toMins(t.times[stn])}))
    .filter(t=>t.stM!==null)
    .sort((a,b)=>a.stM-b.stM);
  const ni=rows.findIndex(r=>r.stM>=nowM);
  useEffect(()=>{if(ref.current)ref.current.scrollIntoView({block:'center',behavior:'smooth'});},[stn,dir]);
  return <div style={scrollS}>
    <div style={{marginBottom:8}}>
      <div style={{fontSize:11,color:C.muted,marginBottom:4,fontWeight:600}}>역 선택</div>
      <select style={selS} value={stn} onChange={e=>setStn(e.target.value)}>
        {ALL_STATIONS.map(s=><option key={s} value={s}>{s}역{TRANSFERS[s]?' 🔗':''}</option>)}
      </select>
    </div>
    ...
    <div style={cardS}>
      <SecHead title={`${stn}역 전체 시간표`} count={rows.length}/>
      {rows.map((r,i)=>{
...
// ── 탭3: 경로 검색 ────────────────────────────────────────────
function RouteTab({nowM,hol,selectedStn}) {
  const [from,setFrom]=useState(selectedStn||'경산');
  const [to,setTo]=useState('동대구');
  // selectedStn 바뀌면 from 동기화 (탭 전환 시)
  useEffect(()=>{ if(selectedStn) setFrom(s=>s); },[]);
      ...
      <div style={{padding:'12px 14px'}}>
        <div style={{display:'flex',gap:8,alignItems:'flex-end',marginBottom:10}}>
          <div style={{flex:1}}>
            ...
            <select style={selS} value={from} onChange={e=>{setFrom(e.target.value);setDone(false);}}>
...
function MapTab({selectedStn, nowM, hol}) {
  const [selMap, setSelMap] = useState(selectedStn||"동대구");

  // selectedStn(다음열차 탭 선택값)이 바뀌면 동기화
  ...
  return <div style={scrollS}>
      ...
      <div style={{padding:"16px 8px 12px", overflowX:"auto"}}>
        <div style={{display:"flex", alignItems:"center", minWidth:360, position:"relative", paddingTop:32, paddingBottom:32}}>
          ...
          {ROUTE_STNS.map((s,i)=>{
            ...
            const labelUp = i%2===0;
            return (
              <div key={s.n} onClick={()=>setSelMap(s.n)}
    ...
    <div style={cardS}>
      <div style={{padding:'9px 14px', background:C.surf, borderBottom:`1px solid ${C.border}`,
        display:'flex', justifyContent:'space-between', alignItems:'center'}}>
        <span style={{fontWeight:700,fontSize:14}}>🚄 {selMap}역 다음 열차</span>
        <span style={{fontSize:10,color:C.muted}}>현재 시각 기준</span>
      </div>
      <div style={{display:'grid', gridTemplateColumns:'1fr 1fr'}}>
        ...
        <div style={{borderRight:`1px solid ${C.border}`}}>
          ...
          {mapUp.length===0
            ? <div style={{padding:'14px 12px',fontSize:12,color:C.muted,textAlign:'center'}}>운행 종료</div>
            : mapUp.map((t,i)=>{
  ...
  return <>
    ...
    <div style={{fontFamily:"'Noto Sans KR','Apple SD Gothic Neo',sans-serif",
      ...
      <div style={{flex:1,display:'flex',flexDirection:'column',overflow:'hidden'}}>
        {tab===0&&<NextTab nowM={nowM} nowSec={nowSec} hol={hol} onStnChange={setSelectedStn}/>}
        {tab===1&&<TTTab  nowM={nowM} hol={hol}/>}
        {tab===2&&<RouteTab nowM={nowM} hol={hol} selectedStn={selectedStn}/>}
        {tab===3&&<MapTab selectedStn={selectedStn} nowM={nowM} hol={hol}/>}
      </div>
