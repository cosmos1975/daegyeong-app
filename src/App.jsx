...
function PrevDepartedPopup({ events, onClose }) {
  // events: [{dir:'up'|'down', label:'구미행'|'경산행', remSec:number}]
  ...
  return (
    <div style={{
      ...
      paddingTop:72, gap:10, pointerEvents:'none',
    }}>
      {events.map((ev, idx) => {
        ...
        return (
          <div key={ev.dir} style={{
            ...
            boxShadow: `0 6px 32px ${accent}44`,
            ...
            <div style={{
              ...
              <span style={{fontSize:16,color:'#526275',margin:'0 2px'}}>분</span>
              {String(secs).padStart(2,'0')}
              <span style={{fontSize:16,color:'#526275',margin:'0 2px'}}>초</span>
            </div>
            {/* 서브 */}
            <div style={{
              fontSize:11, color:'#526275', textAlign:'center',
            }}>후 도착 예정</div>
          </div>
        );
      })}
...
function NextTab({nowM, nowSec, hol, onStnChange}) {
  ...
  // 수동으로 닫은 dir 기록 (닫으면 해당 방향 팝업 숨김, 열차 바뀌면 다시 표시)
  const closedRef = useRef({});

  function changeStn(s){ setStn(s); onStnChange&&onStnChange(s); closedRef.current={}; }

  const up=nextAt(stn,'up',nowM,hol), dn=nextAt(stn,'down',nowM,hol);
  const upF=firstAt(stn,'up',hol), dnF=firstAt(stn,'down',hol);
  const xfer=TRANSFERS[stn];

  // 전역 출발 이벤트 계산
  // 각 방향의 첫 번째 열차에 대해:
  //   이전역(prevStn) 출발 시각 <= 현재 시각 < 현재역 도착 시각  → 전역 출발 중
  function calcPrevEvent(trains, dir) {
    if (!trains.length) return null;
    const t = trains[0];
    const order = dir === 'up' ? UP_ORDER : DOWN_ORDER;
    const stnIdx = order.indexOf(stn);
    // 이전역 (상행이면 +1 인덱스, 하행이면 -1 인덱스 — 하지만 order 배열 방향 주의)
    // UP_ORDER: 경산→구미 순, DOWN_ORDER: 구미→경산 순
    // 상행(up): 열차는 UP_ORDER 순으로 이동 → 현재역 이전에 통과한 역은 stnIdx-1 위치
    // 하행(down): 열차는 DOWN_ORDER 순으로 이동 → 현재역 이전에 통과한 역은 stnIdx-1 위치
  ...
  // nowSec가 바뀔 때마다 (=1초마다) 리렌더
  useEffect(()=>{ forceRender(n=>n+1); },[nowSec]);

  const upEv = calcPrevEvent(up, 'up');
  const dnEv = calcPrevEvent(dn, 'down');
  const popupEvents = [upEv, dnEv].filter(Boolean);

  function handleClose(dir) {
    const trains = dir === 'up' ? up : dn;
    if (trains.length) {
      closedRef.current[dir + '_' + trains[0].id] = true;
      forceRender(n=>n+1);
    }
  }
  ...
  return <>
    ...
    <div style={scrollS}>
      ...
      {[{dir:'up',label:'구미행 (상행)',trains:up,first:upF},{dir:'down',label:'경산행 (하행)',trains:dn,first:dnF}]
        .map(({label,trains,first})=>(
        <div key={label} style={cardS}>
          <SecHead title={`🚄 ${label}`}/>
          {trains.length===0
            ? <div style={{padding:16,textAlign:'center',color:C.muted,fontSize:13}}>
                운행 종료 · 내일 첫차 {fmtM(first)}
              </div>
            : trains.map((t,i)=>{
              const stM=toMins(t.times[stn]);
                ...
                <div>
                  <div style={{display:'flex',alignItems:'baseline',gap:8}}>
                    <span style={{fontFamily:"'Space Mono',monospace",fontSize:22,fontWeight:700,
                      color:i===0?C.accent:C.text,letterSpacing:-0.5}}>{fmtM(stM)}</span>
...
// ── 탭3: 경로 검색 ────────────────────────────────────────────
function RouteTab({nowM,hol}) {
  const [from,setFrom]=useState('경산');
...
function MapTab({selectedStn}) {
  const [selMap, setSelMap] = useState(selectedStn||"동대구");

  // selectedStn이 바뀌면 동기화
    ...
    <div style={cardS}>
      ...
      <div style={{padding:"12px 14px"}}>
        <div style={{display:"grid", gridTemplateColumns:"1fr 1fr", gap:8, marginBottom:10}}>
          {fares.map(({n,fare})=>(
            <div key={n} style={{background:C.surf,borderRadius:8,padding:"8px 10px",border:`1px solid ${C.border}`}}>
              <div style={{fontSize:10,color:C.muted}}>{selMap} ↔ {n}</div>
              <div style={{fontSize:14,fontWeight:700,color:C.text}}>{fmtFare(fare)}</div>
            </div>
          ))}
        </div>
...
//  ROOT
// ═══════════════════════════════════════════════════════════════
const TABS=[{icon:'🚄',label:'다음열차'},{icon:'📋',label:'시간표'},
  ...
  return <>
    ...
    <div style={{fontFamily:"'Noto Sans KR','Apple SD Gothic Neo',sans-serif",
      ...
      <div style={{flex:1,display:'flex',flexDirection:'column',overflow:'hidden'}}>
        ...
        {tab===2&&<RouteTab nowM={nowM} hol={hol}/>}
        {tab===3&&<MapTab selectedStn={selectedStn}/>}
      </div>
