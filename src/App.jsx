...
// ═══════════════════════════════════════════════════════════════
//  날짜 선택 컴포넌트 (fixed 포지션 — overflow:hidden 무관하게 동작)
// ═══════════════════════════════════════════════════════════════
  ...
  return (
    <>
      <button ref={btnRef} onClick={()=>setOpen(o=>!o)} style={{
        ...
        padding:'7px 12px',cursor:'pointer',color:C.text,fontFamily:'inherit',
      ...
      {open&&(
        <div ref={popRef} style={{
          ...
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:10}}>
            <button onClick={prevMonth} style={{background:'none',border:'none',color:C.text,
              cursor:'pointer',fontSize:16,padding:'4px 8px'}}>◀</button>
            <span style={{fontWeight:700,fontSize:13,color:C.text}}>
              {viewYear}년 {viewMonth+1}월
            </span>
            <button onClick={nextMonth} style={{background:'none',border:'none',color:C.text,
              cursor:'pointer',fontSize:16,padding:'4px 8px'}}>▶</button>
          </div>
          {/* 요일 헤더 */}
            ...
            {cells.map((d,i)=>{
              ...
              return (
                <button key={ds} onClick={()=>{onChange(d);setOpen(false);}} style={{
                  ...
                  border:`1px solid ${isSel?C.accent:isToday?C.muted:'transparent'}`,
                  borderRadius:6,padding:'5px 2px',cursor:'pointer',fontSize:12,
                  fontWeight:isSel||isToday?700:400,fontFamily:'inherit',
                  transition:'all .1s',
                }}>
                  {d.getDate()}
                </button>
...
function NextTab({nowM, nowSec, hol, selectedStn, onStnChange, selectedDate, onDateChange}) {
  ...
  const effectiveNowM = isToday ? nowM : 0;
      ...
      <div style={{marginBottom:8}}>
        <div style={{display:'flex',alignItems:'center',justifyContent:'center',gap:8,marginBottom:6}}>
          ...
          <div style={{position:'relative'}}>
            <select
              value={stn}
              onChange={e=>changeStn(e.target.value)}
              style={{
        ...
        <div style={{display:'flex',justifyContent:'center',alignItems:'center',gap:8}}>
          ...
          <span style={{fontSize:11,color:dateHol?C.blue:C.green,fontWeight:700,
            background:dateHol?C.blue+'22':C.green+'22',borderRadius:6,padding:'3px 8px',
            border:`1px solid ${dateHol?C.blue:C.green}44`}}>
            {dateHol?'주말·공휴일':'평일'}
          </span>
        </div>
      </div>
...
function TTTab({nowM, hol, selectedStn, onStnChange, selectedDate, onDateChange}) {
  ...
  const upNi=upRows.findIndex(r=>r.stM>=effectiveNowM);
  const dnNi=dnRows.findIndex(r=>r.stM>=effectiveNowM);
  ...
  function ColRows({rows,ni,accentCol,colRef}){
    return rows.map((r,i)=>{
      const past=r.stM<effectiveNowM, isNext=i===ni;
      return <div key={r.id} ref={isNext?colRef:null} style={{
        ...
        {isNext&&<span style={{fontSize:9,color:accentCol,fontWeight:700,marginTop:1,
          ...
          {r.stM-effectiveNowM<=0?'출발중':r.stM-effectiveNowM<=2?'곧출발':`${r.stM-effectiveNowM}분후`}
        </span>}
        {!isNext&&!past&&<span style={{fontSize:9,color:C.muted,marginTop:1}}>{r.stM-effectiveNowM}분후</span>}
  ...
  return <div style={{...scrollS,padding:'8px 10px',paddingBottom:80,display:'flex',flexDirection:'column'}}>
    {/* sticky 헤더: 역명 + 날짜 + sticky 현재열차 */}
    <div style={{
      position:'sticky',top:0,zIndex:10,
      background:C.surf,borderRadius:10,
      border:`1px solid ${C.border}`,
      marginBottom:8,padding:'8px 10px',flexShrink:0,
    }}>
      {/* 역명 깜빡임 + 역 선택 */}
      <div style={{display:'flex',alignItems:'center',justifyContent:'center',gap:8,marginBottom:6}}>
        <div style={{textAlign:'center',fontSize:22,fontWeight:700,
          color:C.accent,animation:'pulse 1.5s ease-in-out infinite',
          fontFamily:"'Space Mono',monospace",letterSpacing:-0.5}}>
          {stn}역
        </div>
        <div style={{position:'relative'}}>
    ...
    <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:0,
      ...
      <div style={{
        ...
        padding:'7px 4px',background:'#FF7A0018',
      ...
      <div style={{
        position:'sticky',top:0,zIndex:5,
        padding:'7px 4px',background:'#3B82F618',
...
function RouteTab({nowM, hol, selectedStn, onStnChange, selectedDate, onDateChange}) {
  ...
  // nowM 바뀔 때 오늘이면 재계산
  useEffect(()=>{if(isToday&&searched)forceUpdate(n=>n+1);},[nowM]);
  ...
  return <div style={scrollS}>
    <div style={cardS}>
      <div style={{padding:'12px 14px'}}>
        <div style={{display:'flex',gap:8,alignItems:'flex-end',marginBottom:10}}>
          <div style={{flex:1}}>
            <div style={{fontSize:15,color:C.text,marginBottom:5,fontWeight:800,textAlign:'center'}}>출발</div>
          ...
          <div style={{flex:1}}>
            <div style={{fontSize:15,color:C.text,marginBottom:5,fontWeight:800,textAlign:'center'}}>도착</div>
            <select style={{...selS,textAlign:'center',fontSize:15,fontWeight:700}}
    ...
    {searched&&!same&&(
      <div style={cardS}>
        <SecHead title={`${from} → ${to} (${dateHol?'주말·공휴일':'평일'})`}
          count={res.length}/>
        {res.length===0
...
function MapTab({selectedStn,nowM,hol,onStnChange,selectedDate,onDateChange}){
  ...
  const isToday=dateToStr(selectedDate)===dateToStr(new Date());
  const effectiveNowM=isToday?nowM:0;
  useEffect(()=>{if(isToday)forceUpdate(n=>n+1);},[nowM]);

  const fares=ROUTE_STNS.filter(s=>s.n!==selMap).map(s=>({n:s.n,fare:calcFare(selMap,s.n)}));
  const mapUp=nextAt(selMap,'up',effectiveNowM,dateHol,3);
  const mapDn=nextAt(selMap,'down',effectiveNowM,dateHol,3);
  const holLabel=dateHol?'주말·공휴일':'주중';
  ...
  return <div style={scrollS}>
    ...
    <div style={cardS}>
      <div style={{padding:'9px 14px',background:C.surf,borderBottom:`1px solid ${C.border}`,
        ...
        <span style={{fontWeight:800,fontSize:15,letterSpacing:-0.3,textAlign:'center'}}>대경선 노선도 (터치하여 역 선택)</span>
      </div>
    ...
    <div style={cardS}>
      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr'}}>
        <div style={{borderRight:`1px solid ${C.border}`}}>
          {mapUp.length===0
            ? <div style={{padding:'14px 8px',fontSize:22,color:C.muted,textAlign:'left'}}>운행 종료</div>
            : mapUp.map((t,i)=>{
              const stM=toMins(t.times[selMap]);
              const diff=stM-effectiveNowM;
              return <div key={t.id} style={{padding:'8px 10px',borderBottom:`1px solid ${C.border}`,
                ...
                <div style={{fontSize:15,color:C.muted,marginTop:1}}>
        ...
        <div>
          {mapDn.length===0
            ? <div style={{padding:'14px 8px',fontSize:22,color:C.muted,textAlign:'right'}}>운행 종료</div>
            : mapDn.map((t,i)=>{
              const stM=toMins(t.times[selMap]);
              const diff=stM-effectiveNowM;
              return <div key={t.id} style={{padding:'8px 10px',borderBottom:`1px solid ${C.border}`,
    ...
    <div style={cardS}>
      <div style={{padding:'9px 14px',background:C.surf,borderBottom:`1px solid ${C.border}`,
        display:'flex',justifyContent:'center',alignItems:'center'}}>
        <span style={{fontWeight:700,fontSize:14,textAlign:'center'}}>{selMap}역 출발 요금 (교통카드)</span>
      </div>
  ...
  function HolDot({active,label}){
    return(
      <span style={{display:'flex',alignItems:'center',gap:3,
        padding:'1px 4px',borderRadius:10,fontFamily:'inherit'}}>
  ...
  return <>
    ...
    <div style={{fontFamily:"'Noto Sans KR','Apple SD Gothic Neo',sans-serif",
      ...
      <div style={{flex:1,display:'flex',flexDirection:'column',overflow:'hidden'}}>
        {tab===0&&<NextTab nowM={nowM} nowSec={nowSec} hol={hol} selectedStn={selectedStn} onStnChange={setSelectedStn} selectedDate={selectedDate} onDateChange={setSelectedDate}/>}
        {tab===1&&<TTTab nowM={nowM} hol={hol} selectedStn={selectedStn} onStnChange={setSelectedStn} selectedDate={selectedDate} onDateChange={setSelectedDate}/>}
        {tab===2&&<RouteTab nowM={nowM} hol={hol} selectedStn={selectedStn} onStnChange={setSelectedStn} selectedDate={selectedDate} onDateChange={setSelectedDate}/>}
        {tab===3&&<MapTab selectedStn={selectedStn} nowM={nowM} hol={hol} onStnChange={setSelectedStn} selectedDate={selectedDate} onDateChange={setSelectedDate}/>}
      </div>
